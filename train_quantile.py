...
for lr, batch_size, num_epochs in hyperparameter_combinations:
    for model_name in ['STGCN']:  # 可以扩展为多个模型
        train_loss_list = []
        val_loss_list = []
        test_loss_list = []
        ...
        adj_mx = tools.train_tools.expand_adjacency_matrix(np.load(config['adj_mx_file']), config['num_nodes'])[valid_grid][:, valid_grid]
        ...
        for epoch in range(config['num_epochs']):
            ...
            if val_loss < best_val_loss:
                ...
                model_save_path = f'models/{model_name}_lr{lr}_bs{batch_size}_epochs{num_epochs}_epoch{epoch+1}_val_loss{val_loss:.4f}.pth'
        ...
        # 保存最终模型和配置
        save_dir = f"final_models/QR/{model_name}_lr{lr}_bs{batch_size}_epochs{num_epochs}"
...
# for month,model_name in product([48],['STGCN']):
#     for _ in range(1):
#         train_loss_list=[]
#         val_loss_list=[]
#         test_loss_list=[]
...
#         #     model = causal_model.CausalModel(AGCRN.AGCRN,config,adj_mx)
#         # elif config['model_name']=='gwnet':
#         #     model = causal_model.CausalModel(GWNET.gwnet,config,adj_mx)
#         elif config['model_name']=='STGCN':
#             if mode=='any_quantile':
#                 model=causal_model.Causal_CDF_model(STGCN.STGCN,config,adj_mx)
#             elif mode=='quantile' or mode=='quantile':
#                 model = causal_model.CausalModel_quantile_regress(STGCN.STGCN,config,adj_mx)
#             elif mode=='laplace':
#                 model = causal_model.CausalModel_lp(STGCN.STGCN,config,adj_mx)

#             # model=causal_model.CausalModel_MCdropout(STGCN.STGCN,config,adj_mx)
#         # elif config['model_name']=='MTGNN':
#         #     model = causal_model.CausalModel(MTGNN.MTGNN,config,adj_mx)
...
#             if val_loss < best_val_loss:
#                 best_val_loss = val_loss
#                 best_model_save_path = f'{config["model_dir"]}/epoch{epoch+1}_val_loss{val_loss:.4f}_{mode}'+'.pth'
...
#             # print(f' Test Loss: {test_loss:.3f}, '
#             #             f'MAE: {test_mae:.3f}, RMSE: {test_rmse:.3f}, Coverage Rate: {coverage_rate:.4f}, '
#             #             f'Mean CI Length: {mean_ci_length:.3f}, train_time: {train_time:.3f}')
...
#         save_dir = f'final_models/{mode}_{config["dataset_name"]}'
#         os.makedirs(save_dir, exist_ok=True)
...
#         # 再次评估测试数据（如果需要）
#         test_loss, test_mae, test_rmse, coverage_rate, mean_ci_length = evaluate(model, test_loader, criterion, device, scaler,mode=mode) 
       
#         print(f' Test Loss: {test_loss}, '
#                         f'MAE: {test_mae}, RMSE: {test_rmse}, Coverage Rate: {coverage_rate:.4f}, '
#                         f'Mean CI Length: {mean_ci_length}, train_time: {train_time}')
#         results = [_+1, epoch+1, train_loss, best_val_loss, test_mae, test_rmse, dict2srt(config)]
#         # np.save('loss'+dict2srt(config)+'.npy',np.array([train_loss_list,val_loss_list,test_loss_list]))
#         # with open('result_{}_{}.csv'.format(config['model_name'],config['dataset_name']), mode='a', newline='') as file:
